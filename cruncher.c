...
void query(unsigned short qid, unsigned short artist, unsigned short areltd[], unsigned short bdstart, unsigned short bdend) {
	...
	unsigned char score;
	//char* p_fan = malloc(person_length/sizeof(Person));
	...
	for (person_offset = 0; person_offset < person_length/sizeof(Person); person_offset++) {
		...
		// get the score from each person P
	...
	for (person_offset = 0; person_offset < person_length/sizeof(Person); person_offset++) {
		...
		//if(p_fan[person_offset] == 0)continue;
	...
	// friend must already like the artist
		...
		for (knows_offset = friend->knows_first; 
			...
			knows_offset++) {
	     	...
	     	// Consider a new offset for the friends
			knows_offset2 = knows_map[knows_offset];
	
			// Set each Person F
			person = &person_map[knows_offset2];

			// Person and friend must live in same location
			//if (person->location != friend->location) continue; 


			// Check if friends score is sufficient
			...
			// 	knows_offset2++) {
				...
				//if (knows_offset2 == person_offset) {
