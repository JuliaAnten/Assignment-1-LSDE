...
void* mmapopen(char* filename, byteoffset *filelen, bool write) {
    ...
    *filelen = sbuf.st_size;
    ...
    // If it can, it will map everything sequential
    int ret = madvise (mapaddr,
             0,
             MADV_SEQUENTIAL);
    if(ret < 0){
    	perror("madvise");
...
// Write the file back to the file systems disk
int mmapwritefile(void *addr, size_t len, int flags){
	return msync(addr, len, flags);
}

int mmapremovefile(void *addr, size_t len){
